...
class SaveUniverseWorker(QThread):
    done  = Signal(str)   # "ok" or error
    error = Signal(str)

    def __init__(self, data: dict, universe_id: int = None):
        super().__init__()
        self.data        = data
        self.universe_id = universe_id   # None = create, int = update
...
class UniverseFormPanel(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self._edit_id = None   # None = create mode
        self._worker  = None
        ...
        self._title.setStyleSheet(
            "color: #00ADB5; font-size: 17px; font-weight: 800; "
            "background: transparent; border: none;"
        )
        ...
        self.canon_combo.setStyleSheet(field_style)
        ...
        self.score_slider.setStyleSheet("""
            ...
            QSlider::handle:horizontal {
                background: #00ADB5; width: 16px; height: 16px;
        ...
        self.score_val.setStyleSheet(
            "color: #00ADB5; font-size: 13px; font-weight: 700; "
            "background: transparent; border: none;"
        )
...
class UniversesViewWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        """)
        self._search_input.returnPressed.connect(self._load)
    ...
    def _on_search_changed(self, text: str):
        # Debounce — only search if ≥2 chars or empty
    ...
    def _on_loaded(self, universes: list):
        ...
        self._status_lbl.setText(
            f"{count} universe{'s' if count != 1 else ''}"
        )
